...
class Ques_Gen_Book:
    ...
    def __init__(self):
        ...
        'Invoice':['inv_no','issue_date','return_date'],
    ...
    def lvl_1_ques(self):
        ...
        ques = f"What is the customer ID of Customer having last name '{cust_lname[0]}' or '{cust_lname[1]}'"
        query = f"SELECT cust_id FROM Customer WHERE lname like '{cust_lname[0]}' OR lname like '{cust_lname[1]}'"
        ques_list.append(ques)
        query_list.append(query)
    ...
    def lvl_2_ques(self):
        ...
        ques = f"Retrieve all Customer details having their first name starting with '{fname_starting}'"
        query = f"SELECT * FROM Customer WHERE fname like '{fname_starting}%'"
        ques_list.append(ques)
        query_list.append(query)

        lname_ending = random.choice(lname_ends_with)
        ques = f"Get the area of the Customer whose last name ends with '{lname_ending}'"
        query = f"SELECT area FROM Customer WHERE lname like '%{lname_ending}'"
        ques_list.append(ques)
        query_list.append(query)

        fname_ending = random.choice(fname_ends_with)
        ques = f"Which Customers have '{fname_ending}' at the end of their first name? Get all details."
        query = f"SELECT * FROM Customer WHERE fname like '%{fname_ending}'"
        ques_list.append(ques)
        query_list.append(query)

        lname_starting = random.choice(lname_starts_with)
        ques = f"List the phone number of Customers who have the letter '{lname_starting}' at the beginning of their last name."
        query = f"SELECT phone_no FROM Customer WHERE lname like '{lname_starting}%'"
        ques_list.append(ques)
        query_list.append(query)
        ...
        ques = f"What are the names of the Books with author names starting with '{authorname_starting}?'"
        query = f"SELECT title FROM Book where author like '{authorname_starting}%'"
        ques_list.append(ques)
        query_list.append(query)

        authorname_ending = random.choice(author_ends_with)
        ques = f"Get all rows of Books with '{authorname_ending}' at the end of the author name?"
        query = f"SELECT * FROM Book where author like '%{authorname_ending}'"
        ques_list.append(ques)
        query_list.append(query)

        title_starting = random.choice(title_starts_with)
        ques = f"What is the name of the author who wrote the Book whose name starts with '{title_starting}?'"
        query = f"SELECT author FROM Book where title like '{title_starting}%'"
        ques_list.append(ques)
        query_list.append(query)

        title_ending = random.choice(title_ends_with)
        ques = f"What is the price of the books whose name ends with '{title_ending}?'"
        query = f"SELECT price FROM Book where title like '%{title_ending}'"
        ques_list.append(ques)
        query_list.append(query)
